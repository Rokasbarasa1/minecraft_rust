...
Added an rendering of new chunks when the camera is moving. Now the world can be pretty big. Had a lot of trouble with this one because i had made mistakes earlier in rendering objects. New chunks are generated by shifting the entire chunk vector to one direction and then reconstructing one of them in place. Only block id's and positions are changed to improve performance. The performance impact is big if the chunks are of size 16. I will have to improve it after i fix some more bugs with the rest of the source code.
